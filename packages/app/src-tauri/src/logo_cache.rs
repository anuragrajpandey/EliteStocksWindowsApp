...
impl LogoCacheManager {
    ...
    /// Batch pre-fetch multiple logo URLs concurrently in chunks
    pub async fn prefetch_logos(&self, urls: Vec<String>) -> usize {
        let mut count = 0;
        let chunk_size = 50;
        ...
        for chunk in urls.chunks(chunk_size) {
            ...
            for url in chunk {
                ...
                let manager_client = self.client.clone();
                ...
                tasks.push(tokio::spawn(async move {
                    let file_path = cache_dir.join(format!("{}.{}", hash, fallback_ext));
                    if file_path.exists() {
                        return false;
                    }

                    match manager_client.get(&url_clone).send().await {
                        Ok(resp) if resp.status().is_success() => {
                            if let Ok(bytes) = resp.bytes().await {
                                let fetched = bytes.to_vec();
                                let size = fetched.len() as u64;
                                if fs::write(&file_path, &fetched).await.is_ok() {
                                        ...
                                        let _ = conn.execute(
                                            "INSERT INTO logo_cache_meta (url_hash, original_url, last_fetched, fail_count, last_failed_at, file_ext, file_size)
                                             ...
                                             ON CONFLICT(url_hash) DO UPDATE SET
                                                 ...
                                                 file_ext = excluded.file_ext,
                                                 file_size = excluded.file_size;",
                                            params![hash, url_clone, now, fallback_ext, size],
                                        );
                                    }
                                    return true;
                                }
        ...
        info!("[LogoCache] Prefetched {} new logos", count);
        count
    }
